...
void Adafruit_DRV2605::setWaveform(uint8_t slot, uint8_t w) {
  ...
  writeRegister7(DRV2605_REG_WAVESEQ1+slot, w);
}

void Adafruit_DRV2605::selectLibrary(uint8_t lib) {
  writeRegister8(DRV2605_REG_LIBRARY, lib);
  writeRegister7(DRV2605_REG_LIBRARY, lib);
}

void Adafruit_DRV2605::go(uint8_t g) {

  if(g==0){
  writeRegister8(DRV2605_REG_GO, 1);
  }
  else if(g==1){
  writeRegister7(DRV2605_REG_GO, 1);
  }
}

void Adafruit_DRV2605::setMode(uint8_t mode) {
  writeRegister8(DRV2605_REG_MODE, mode);
  writeRegister7(DRV2605_REG_MODE, mode);
}

void Adafruit_DRV2605::setRealtimeValue(uint8_t rtp) {
  writeRegister8(DRV2605_REG_RTPIN, rtp);
  writeRegister7(DRV2605_REG_RTPIN, rtp);
}
...
uint8_t Adafruit_DRV2605::readRegister7(uint8_t reg) {
  uint8_t x ;
...
void Adafruit_DRV2605::writeRegister8(uint8_t reg, uint8_t val) {
  ...
  //}
  //else if(dr==1){
  void Adafruit_DRV2605::writeRegister7(uint8_t reg, uint8_t val) {
   Wire1.beginTransmission(DRV2605_ADDR);
   Wire1.write((byte)reg);
