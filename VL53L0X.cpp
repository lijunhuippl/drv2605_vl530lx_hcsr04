...
// The Arduino two-Wire2 interface uses a 7-bit number for the address,
...
void VL53L0X::writeReg(uint8_t reg, uint8_t value)
{
  ...
  Wire2.write(reg);
  Wire2.write(value);
  last_status = Wire2.endTransmission();
}
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
  Wire2.beginTransmission(address);
  Wire2.write(reg);
  Wire2.write((value >> 24) & 0xFF); // value highest byte
  Wire2.write((value >> 16) & 0xFF);
  Wire2.write((value >>  8) & 0xFF);
  Wire2.write( value        & 0xFF); // value lowest byte
  last_status = Wire2.endTransmission();
}
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
  ...
  value  = (uint16_t)Wire2.read() << 8; // value high byte
  value |=           Wire2.read();      // value low byte
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
  ...
  value  = (uint32_t)Wire2.read() << 24; // value highest byte
  value |= (uint32_t)Wire2.read() << 16;
  value |= (uint16_t)Wire2.read() <<  8;
  value |=           Wire2.read();       // value lowest byte
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
  ...
  Wire2.requestFrom(address, count);
